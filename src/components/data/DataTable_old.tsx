...
interface DataTableProps<T extends GridValidRowModel> {
  // 添加约束 T 继承自 GridValidRowModel
  columns: GridColDef[]; // 表格列定义
  fetchData: (page: number, pageSize: number) => Promise<PaginatedData<T>>; // API 获取数据的函数
...
export default function DataTable<T extends GridValidRowModel>({
  ...
  useEffect(() => {
    loadData(page, pageSize); // 初始加载数据
  }, [page, pageSize, loadData]); // 添加 loadData 到依赖数组
