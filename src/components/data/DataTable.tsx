...
interface DataTableProps<T extends GridValidRowModel> {
  ...
  fetchData: (
    ...
    pageSize: number,
    queryParams: any
  ) => Promise<PaginatedData<T>>; // API 获取数据的函数，新增 queryParams 参数
  getRowIdKey?: string; // 可选的自定义字段名，用于指定唯一 id 字段
  queryParams?: any; // 可选的查询条件参数
}
...
export default function DataTable<T extends GridValidRowModel>({
  ...
  const loadData = useCallback(
    async (page: number, pageSize: number, queryParams: any) => {
      setLoading(true);
      try {
        const data = await fetchData(page, pageSize, queryParams); // 传递查询参数
  ...
  useEffect(() => {
    loadData(page, pageSize, queryParams); // 初始加载数据时传递查询参数
  }, [page, pageSize, loadData, queryParams]); // 添加 queryParams 到依赖数组
